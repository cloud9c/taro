...
const System = {
	...
	collision: {
		...
		resolveCollision(colA, colB, res) {
			...
			if (!(colA.isTrigger || colB.isTrigger) && (colA.hasOwnProperty('Physics') || colB.hasOwnProperty('Physics'))) {
				const bounciness = (colA.material.bounciness + colB.material.bounciness)/2;
				const staticFriction = (colA.material.staticFriction + colB.material.staticFriction)/2;
				const dynamicFriction = (colA.material.dynamicFriction + colB.material.dynamicFriction)/2;
				...
				if (colA.hasOwnProperty('Physics')) {
					...
					res.dir.applyQuaternion(colA.Object3D.getWorldQuaternion(this.quat));
					colA.Physics.velocity.projectOnPlane(res.dir);
				}

				if (colB.hasOwnProperty('Physics')) {
					colB.Object3D.position.add(res.dir.multiplyScalar(res.dist));
					res.dir.applyQuaternion(colB.Object3D.getWorldQuaternion(this.quat).inverse());
...
function initMenu(cached, config) {
	...
	function textOnClick() {
		System.input.controls[this.name] = config.controls[this.name] = this.value = '';
	}
	...
	for (const element of document.querySelectorAll('.setting input:not([type=number]), .setting select')) {
		...
		switch (element.type) {
			...
			case 'text':
				...
				element.nextElementSibling.addEventListener('click', textOnClick);
				break;
			default:
