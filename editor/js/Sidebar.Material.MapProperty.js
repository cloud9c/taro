...
import { UITexture } from './libs/ui.taro.js';
...
function SidebarMaterialMapProperty( editor, property, name ) {
	...
	function onChange() {
		...
		if ( material[ property ] !== newMap ) {
			...
			if ( newMap !== null ) {
				...
				if ( property === 'envMap' ) newMap.mapping = TARO.EquirectangularReflectionMapping;
	...
	function onMapChange( texture ) {
		...
		if ( texture !== null ) {
			...
			if ( texture.isDataTexture !== true && texture.encoding !== TARO.sRGBEncoding ) {

				texture.encoding = TARO.sRGBEncoding;
