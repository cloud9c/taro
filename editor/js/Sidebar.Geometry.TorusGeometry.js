...
function GeometryParametersPanel( editor, object ) {
	...
	const arc = new UINumber( parameters.arc * TARO.MathUtils.RAD2DEG ).setStep( 10 ).onChange( update );
	...
	function update() {
		...
		editor.execute( new SetGeometryCommand( editor, object, new TARO.TorusGeometry(
			radius.getValue(),
			tube.getValue(),
			radialSegments.getValue(),
			tubularSegments.getValue(),
			arc.getValue() * TARO.MathUtils.DEG2RAD
		) ) );
