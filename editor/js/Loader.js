...
function Loader( editor ) {
	...
	this.loadFiles = function ( files, filesMap ) {
		...
		if ( files.length > 0 ) {
			...
			const manager = new TARO.LoadingManager();
		...
		switch ( extension ) {
...

			{
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					const { TAROMFLoader } = await import( '../../examples/jsm/loaders/3MFLoader.js' );

					const loader = new TAROMFLoader();
					const object = loader.parse( event.target.result );
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					loader.decodeDracoFile( contents, function ( geometry ) {
						...
						if ( geometry.index !== null ) {
							...
							object = new TARO.Mesh( geometry, material );
							object.name = filename;

						} else {

							const material = new TARO.PointsMaterial( { size: 0.01 } );
							material.vertexColors = geometry.hasAttribute( 'color' );

							object = new TARO.Points( geometry, material );
							object.name = filename;
...

			{
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					mesh.mixer = new TARO.AnimationMixer( mesh );
					mesh.name = filename;
...

			{
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					for ( let i = 0; i < paths.length; i ++ ) {
						...
						const material = new TARO.MeshBasicMaterial( {
							color: path.color,
							depthWrite: false
						} );
						...
						for ( let j = 0; j < shapes.length; j ++ ) {
							...
							const geometry = new TARO.ShapeGeometry( shape );
							const mesh = new TARO.Mesh( geometry, material );
...

			{
				...
				reader.addEventListener( 'load', async function ( event ) {
					...
					const material = new TARO.PointsMaterial();
					material.vertexColors = geometry.hasAttribute( 'color' );

					const points = new TARO.Points( geometry, material );
	...
	function handleJSON( data ) {
		...
		switch ( data.metadata.type.toLowerCase() ) {
...

			{
				...
				const loader = new TARO.BufferGeometryLoader();
				const result = loader.parse( data );

				const mesh = new TARO.Mesh( result );
	...
	function isGLTF1( contents ) {
		...
		if ( typeof contents === 'string' ) {
			...
			const magic = TARO.LoaderUtils.decodeText( new Uint8Array( contents, 0, 4 ) );
			...
			if ( magic === 'glTF' ) {
				...
				resultContent = TARO.LoaderUtils.decodeText( new Uint8Array( contents ) );
