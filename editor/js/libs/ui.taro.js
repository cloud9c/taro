		...
		function loadFile( file ) {
			...
			if ( extension === 'hdr' || extension === 'pic' ) {
				...
				reader.addEventListener( 'load', function ( event ) {
					...
					const texture = new TARO.CanvasTexture( canvas, mapping );
				...
				reader.addEventListener( 'load', function ( event ) {
					...
					image.addEventListener( 'load', function () {
						...
						const texture = new TARO.Texture( this, mapping );
						texture.sourceFile = file.name;
	...
	constructor() {
		...
		function onTextureChanged() {
			...
			if ( images.length === 6 ) {
				...
				const cubeTexture = new TARO.CubeTexture( images );
		...
		if ( cubeTexture !== null ) {
			...
			if ( Array.isArray( images ) === true && images.length === 6 ) {
				...
				for ( let i = 0; i < images.length; i ++ ) {
					...
					const texture = new TARO.Texture( image );
...
class UIPoints2 extends UIPoints {
	...
	getValue() {
		...
		for ( let i = 0; i < this.pointsUI.length; i ++ ) {
			...
			points.push( new TARO.Vector2( pointUI.x.getValue(), pointUI.y.getValue() ) );
			++ count;
...
class UIPoints3 extends UIPoints {
	...
	getValue() {
		...
		for ( let i = 0; i < this.pointsUI.length; i ++ ) {
			...
			points.push( new TARO.Vector3( pointUI.x.getValue(), pointUI.y.getValue(), pointUI.z.getValue() ) );
			++ count;
			pointUI.lbl.setValue( count );
...
function renderToCanvas( texture ) {
	...
	if ( renderer === undefined ) {
		...
		renderer = new TARO.WebGLRenderer();
		renderer.outputEncoding = TARO.sRGBEncoding;
	...
	const camera = new TARO.OrthographicCamera( - 1, 1, 1, - 1, 0, 1 );

	const material = new TARO.MeshBasicMaterial( { map: texture } );
	const quad = new TARO.PlaneGeometry( 2, 2 );
	const mesh = new TARO.Mesh( quad, material );
	scene.add( mesh );
