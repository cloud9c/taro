	...
	constructor( editor ) {
		...
		const onSessionStarted = async ( session ) => {
			...
			if ( group === null ) {
				...
				const geometry = new TARO.BufferGeometry();
				geometry.setFromPoints( [ new TARO.Vector3( 0, 0, 0 ), new TARO.Vector3( 0, 0, - 5 ) ] );

				const controller1 = renderer.xr.getController( 0 );
				controller1.add( new TARO.Line( geometry ) );
				group.add( controller1 );

				const controller2 = renderer.xr.getController( 1 );
				controller2.add( new TARO.Line( geometry ) );
