...
function GeometryParametersPanel( editor, object ) {
	...
	const phiStart = new UINumber( parameters.phiStart * TARO.MathUtils.RAD2DEG ).setStep( 10 ).onChange( update );
	...
	const phiLength = new UINumber( parameters.phiLength * TARO.MathUtils.RAD2DEG ).setStep( 10 ).onChange( update );
	...
	const thetaStart = new UINumber( parameters.thetaStart * TARO.MathUtils.RAD2DEG ).setStep( 10 ).onChange( update );
	...
	const thetaLength = new UINumber( parameters.thetaLength * TARO.MathUtils.RAD2DEG ).setStep( 10 ).onChange( update );
	...
	function update() {
		...
		editor.execute( new SetGeometryCommand( editor, object, new TARO.SphereGeometry(
			radius.getValue(),
			widthSegments.getValue(),
			heightSegments.getValue(),
			phiStart.getValue() * TARO.MathUtils.DEG2RAD,
			phiLength.getValue() * TARO.MathUtils.DEG2RAD,
			thetaStart.getValue() * TARO.MathUtils.DEG2RAD,
			thetaLength.getValue() * TARO.MathUtils.DEG2RAD
		) ) );
